...
原型地址：[https://gaagentplatform.pages.dev/dev/init](https://gaagentplatform.pages.dev/dev/init)

## 待确认问题

1.  针对于空间内不同角色有不同的功能，是否改造现有的菜单权限机制？--如果空间管理功能不放在侧边菜单而是放在空间切换器中，可回避此问题。
    
2.  空间内普通用户角色是否可以查看资源广场并申请资源？
...
| --- | --- | --- | --- | --- | --- |
| v1.0 | 2026-07-17 | 郭泉江 | 新建 | 整合三大模块（空间切换与申请、空间运营、空间管理）为一个闭环体系的PRD | 全篇 |
| v1.1 | 2026-07-17 | 郭泉江 | 修改 | 简化个人空间初始化流程：创建账号时自动初始化个人空间；移除 `/dev/init` 单独初始化页；进入开发中心默认定位到上次进入的空间 | 4.1 / 5.1 / 5.2.0 |
...
| B. 空间运营 | 开发中心侧边导航 | `SpaceManagePage` | `WorkspaceContext.currentSpace` |
| C. 空间管理 | 管理中心侧边导航 | `OpsSpacesPage` | 全部空间数据 |

### 2.2 核心价值

1.  **自动初始化**：创建账号时自动创建个人空间，无需用户手动初始化，开箱即用
    
2.  **快速切换**：侧边栏顶部常驻入口，一键打开弹窗，搜索+切换全在 Modal 内完成
    
3.  **空间记忆**：进入开发中心时自动定位到上次访问的空间，减少重复操作
    
4.  **全局联动**：切换后通过 WorkspaceContext 驱动所有页面数据刷新
    
5.  **自主申请**：用户无需离开开发中心即可发起空间申请，降低协作门槛
    
6.  **审批闭环**：申请 → 管理员审批 → 空间启用 → 出现在已加入成员的切换器，全流程可追踪
    
7.  **生命周期管控**：冻结/归档/删除三级差异化管理，ConfirmActionModal 分级确认
...
```mermaid
flowchart TD
    A[用户点击开发中心] --> H1[默认进入上次访问的空间 /dev/workbench]
    H1 --> I[侧边栏渲染空间触发器]
    I --> J{点击触发器}
    J -->|打开弹窗| K[展示可用空间列表]
    J -->|折叠模式| I
...
1.  **入口与空间记忆**：用户点击开发中心时，系统读取上次访问的空间记录，自动定位到该空间的工作台 `/dev/workbench`。侧边栏顶部常驻展示 WorkspaceSwitcher 触发器（展开/折叠模式自适应）。
    
2.  **切换弹窗**：Modal 680px宽，展示启用空间列表（上次访问空间排最前），支持实时搜索过滤
    
3.  **申请新空间**：弹窗底部"申请新的空间"链接→关闭Modal→打开4步骤StepDrawer
    
4.  **申请提交**：构造审核记录（status='待审核'）→写入审核表→关闭抽屉→提示成功
    
5.  **管理员审批**：进入管理中心空间管理Tab2→打开审批详情抽屉（含基本信息/预置资源清单/成员清单/审批须知）→通过/驳回
    
6.  **审批通过**：写入空间表（status='启用'）→审核记录状态变为"已通过"→空间在已加入成员的切换器中可见
    
7.  **审批驳回**：填写驳回原因→审核记录状态变为"已驳回"→进入Tab3审批记录
    
8.  **空间运营**：用户切换进入空间后，可在/dev/space-ops管理基本信息/成员/日志/API Key
    
9.  **空间管控**：管理员在Tab1执行创建空间（4步向导）、编辑详情、冻结/归档/删除等操作
    
10.  **异常分支**：切换器不展示归档/冻结（灰色不可选）空间；待审核和已驳回不在空间表中
...
| --- | --- | --- | --- | --- | --- | --- |
| **P01** | **开发中心/空间切换触发器** | Trigger | 开发中心侧边栏 | 展示当前空间头像+名称，点击打开弹窗；进入开发中心时默认定位到上次访问的空间 | 5.2.1 | ✅ |
...
| **P02** | **开发中心/空间运营页** | List/Tabs | /dev/space-ops | 4Tab：基本信息+成员管理+操作日志+API Key | 5.3.2 | ✅ |
...
| **P03** | **管理中心/空间管理页** | List/Tabs | /manage/space-manage | 3Tab：空间管理+待审批+审批记录 | 5.4.1 | ✅ |
...
#### \[P00\] 个人空间自动初始化（已废弃独立页面）

##### 业务意图

用户在创建账号时，系统自动为其创建个人空间（命名规则：`{用户名}的个人空间`，type=个人空间），无需用户手动初始化。用户点击「开发中心」即可直接进入上次访问的空间开始使用。

##### 实现方式

- 账号注册/创建时，后端同步创建个人空间记录（status=启用）
- 进入开发中心时，读取用户 `lastVisitedSpaceId`，自动 switchSpace 到对应空间
- 移除 `/dev/init` 路由和 `InitPersonalSpacePage` 组件
...
| 选择资源范围 | 选择资源广场所有已上架的资源 | 选择资源广场所有当前用户可见且已上架的资源 |
| 最后一步按钮 | "确认创建" | "提交申请" |
| 提交后状态 | 直接"启用" | "待审核" |
...
| Step 1 | 预置资源 | 从平台预置资源中选择模型、工具、连接器、知识库（多选，可选） |
...
| 空间描述 | TextArea(3行) | 是 | \- | ≤500 | 描述用途和适用范围 |
...
| --- | --- | --- | --- | --- |
| 冻结 | warning | 空间不可进入；已发布的智能体对外服务**继续运行**；可随时恢复启用，数据不受影响 | 否 | 确认冻结 |
| 归档 | info | 空间不可进入、不可操作；归档前请确认空间内无已发布的智能体和已上线资源广场的资源；可恢复，但不轻易操作；适用于长期不活跃或使命完成的空间 | 否 | 确认归档 |
